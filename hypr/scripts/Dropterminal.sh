...
WIDTH_PERCENT=68  # Width as percentage of screen width
HEIGHT_PERCENT=68 # Height as percentage of screen height
X_PERCENT=16      # X position as percentage from left (25% centers a 50% width window)
Y_PERCENT=5       # Y position as percentage from top

# Animation settings
#ANIMATION_DURATION=30 # milliseconds
#SLIDE_STEPS=5
#SLIDE_DELAY=5 # milliseconds between steps
SLIDE_STEPS=1
SLIDE_DELAY=1 # ms
ANIMATION_DURATION=$((SLIDE_STEPS * SLIDE_DELAY))

# Path to default apps config
DEFAULT_APPS="$HOME/.config/hypr/UserConfigs/01-UserDefaults.conf"

# Extract $term value
#TERM_APP=$(grep '^\$term' "$DEFAULT_APPS" | awk -F= '{print $2}' | xargs)
TERM_APP=$(grep '^\$term' "$DEFAULT_APPS" | awk -F= '{print $2}' | cut -d'#' -f1 | xargs)
...
spawn_terminal() {
  ...
  # Use TERM_APP from environment or fallback to kitty
  TERM_APP="${TERM_APP:-kitty}"

  # Fallback: try to find by most recently mapped terminal window
  if [ -z "$new_addr" ] || [ "$new_addr" = "null" ]; then
    new_addr=$(hyprctl clients -j | jq -r \
      --arg TERM_APP "$TERM_APP" \
      'map(select(.class == $TERM_APP)) | sort_by(.focusHistoryID) | .[-1] | .address')
  fi
